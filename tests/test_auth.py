...
def _token(aud="epicvibe"):
    return pyjwt.encode({"iss": "epic", "aud": aud, "exp": int(time.time()) + 300}, KEY, algorithm="RS256")

def _token_no_exp(aud="epicvibe"):
...
async def test_token_without_exp_rejected():
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=_app()), base_url="http://t") as c:
        r = await c.post("/cds-services/epicvibe-patient-view", json=_body(),
                         headers={"Authorization": f"Bearer {_token_no_exp()}"})
    assert r.status_code == 401

async def test_discovery_unauthenticated_even_with_verify_jwt():
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=_app()), base_url="http://t") as c:
